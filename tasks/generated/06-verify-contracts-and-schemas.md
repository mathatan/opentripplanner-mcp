...
- [x] V-C-001 Analyze + Fix `specs/001-opentripplanner-mcp-server/contracts/find_stops.md` against data model, `docs/` and OTP
  - Verify stop fields (id, name, lat/lon, code, modes), search bounding parameters, pagination, and sorting semantics.
- [x] V-C-002 Analyze + Fix `specs/001-opentripplanner-mcp-server/contracts/geocode_address.md` against data model, `docs/` and OTP
...
- [x] V-C-003 Analyze + Fix `specs/001-opentripplanner-mcp-server/contracts/reverse_geocode.md` against data model, `docs/` and OTP
...
- [x] V-C-004 Analyze + Fix `specs/001-opentripplanner-mcp-server/contracts/get_departures.md` against data model, `docs/` and OTP
...
- [x] V-C-005 Analyze + Fix `specs/001-opentripplanner-mcp-server/contracts/plan_trip.md` against data model, `docs/` and OTP
...
- [x] V-C-006 Analyze + Fix `specs/001-opentripplanner-mcp-server/contracts/user_variables.md` against data model, `docs/` and OTP
...
- [x] V-S-001 Analyze + Fix `src/schema/coordinate.ts`
  - Confirm that the schema matches defined contracts and data model.
- [x] V-S-002 Analyze + Fix `src/schema/locationRef.ts`
  - Confirm that the schema matches defined contracts and data model.
- [x] V-S-003 Analyze + Fix `src/schema/geocode.ts`
  - Confirm that the schema matches defined contracts and data model.
...
- [x] V-S-004 Analyze + Fix `src/schema/itinerary.ts`
  - Confirm that the schema matches defined contracts and data model.
...
- [x] V-S-005 Analyze + Fix `src/schema/departure.ts`
  - Confirm that the schema matches defined contracts and data model.
- [x] V-S-006 Analyze + Fix `src/schema/planConstraints.ts`
  - Confirm that the schema matches defined contracts and data model.
...
- [x] V-S-007 Analyze + Fix `src/schema/userVariable.ts`
  - Confirm that the schema matches defined contracts and data model.
- [x] V-S-002 Analyze + Fix `src/schema/locationRef.ts`
  - Confirm that the schema matches defined contracts and data model. Changes: made the preprocess non-mutative, preserved legacy `label` field (no deletion), inferred discriminant when missing, and enforced single-line address; added explicit `label` optional field in variants.

- [x] V-S-007 Analyze + Fix `src/schema/userVariable.ts`
  - Confirm that the schema matches defined contracts and data model. Changes: widened location value acceptance so `value` may contain either `{ coordinate: {lat,lon} }` or legacy `{ lat, lon, name?, address? }`; validation uses `CoordinateSchema` in either case; preserved TTL normalization and transforms.
- [x] V-S-008 Analyze + Fix `src/schema/error.ts`
  - Confirm that the schema matches defined contracts and data model.
...
- Implemented V-S-003: extended `src/schema/geocode.ts` with Pelias fields and `size`/truncation transform (caps at 40, emits `truncated-results` warning).
- Implemented V-S-004: added optional `realtimeState` and `lastRealtimeUpdate` to `LegSchema` in `src/schema/itinerary.ts`.
- Implemented V-S-005: added optional `scheduleType` to `DepartureResponseSchema` and tightened realtimeTime refinement in `src/schema/departure.ts`.
- Implemented V-S-006: added `first`, `searchWindowMinutes`, and `AccessibilityPrefs.wheelchair` to `src/schema/planConstraints.ts`.
- Implemented V-S-008: added `CanonicalErrorCodes` and `superRefine` checks in `src/schema/error.ts` to require `retryAfter` for `rate-limited`.

Files changed: `src/schema/geocode.ts`, `src/schema/itinerary.ts`, `src/schema/departure.ts`, `src/schema/planConstraints.ts`, `src/schema/error.ts`.

Additional changes in this pass:

- Added `src/schema/planTripSchema.ts` (strict input schema; rejects identical from/to; ISO time).
- Fixed `tests/schema/planTrip.schema.test.ts` import to use the new schema.

---

## Schema Tests — Analyze + Fix

- [x] V-T-001 Review `tests/schema/coordinate.schema.test.ts`
  - Ensure schema tests for boundary and invalid cases for lat/lon ranges and precision.
- [x] V-T-002 Review `tests/schema/locationRef.schema.test.ts`
- Ensure schema tests for discriminant enforcement and mutual exclusivity validation failures.
- [x] V-T-003 Review `tests/schema/geocode.schema.test.ts`
...
- [x] V-T-004 Review `tests/schema/itinerary.schema.test.ts`
...
- [x] V-T-005 Review `tests/schema/departure.schema.test.ts`
- Ensure schema tests for realtime delay/cancellation, scheduleType, and timestamp handling.
- [x] V-T-006 Review `tests/schema/planConstraints.schema.test.ts`
...
- [x] V-T-007 Review `tests/schema/userVariable.schema.test.ts`
- Ensure schema tests for TTL behavior, allowed value types, and metadata fields.
- [x] V-T-008 Review `tests/schema/error.schema.test.ts`
...
- [x] V-T-009 Review `tests/schema/findStops.schema.test.ts`
- Ensure schema tests for search parameters, pagination, and sorting cases.
- [x] V-T-010 Review `tests/schema/geocodeAddress.schema.test.ts`
- Ensure schema tests for address input variants, localization, and limits.
- [x] V-T-011 Review `tests/schema/planTrip.schema.test.ts`
...
- [x] V-X-001 Coordinate order and CRS are consistent across all contracts, schemas, and tests.
- [x] V-X-002 Time handling: all timestamps standardized to ISO 8601 with offset; timezone semantics noted.
- [x] V-X-003 Realtime semantics: `scheduleType`, `dataFreshness` fields aligned with `realtime-apis.md` and OTP.
  - Optional-Phase1: Documented semantics; runtime emission kept optional.
- [x] V-X-004 Error taxonomy: codes, messages, hints follow repo rules and are used consistently in contracts and schemas.
- [ ] V-X-005 Enumerations/modes: values and casing align with OTP; unions are exhaustive with `never` checks where applicable. (Deferred)
- [ ] V-X-006 Public contract changes: if any are required, mark as breaking (C7), add tests (C1, C2), and update specs accordingly. (Deferred)
- [x] V-X-007 Units and ranges: meters vs kilometers consistently used (e.g., walkingDistance meters, geocode distanceKm), and numeric bounds enforced.
- [x] V-X-008 Warnings taxonomy: `truncated-results` and `realtime-missing` warnings are documented and validated by schemas.
- [x] V-X-009 Realtime layering: leg-level `realtimeState`/`lastRealtimeUpdate` consistent with response-level `realtimeUsed`/`dataFreshness`.
