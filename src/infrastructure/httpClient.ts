...
export class HttpClient {
    ...
    async request(url: string, opts?: HttpRequestOptions): Promise<any> {
        ...
        if (typeof opts?.simulateStatus === "number" && opts.simulateStatus >= 400) {
            const err = mapHttpError({ status: opts.simulateStatus, headers: opts?.headers, body: opts?.mockResponse });
        ...
        let resp: any;
        try {
            resp = timeout ? await Promise.race([fetchPromise, timeoutPromise]) : await fetchPromise;
        } catch (err: any) {
